...
-- ==========================================
-- DEV WRITE ACCESS (optional)
-- For local testing, you may want to enable INSERT/UPDATE/DELETE for anon & authenticated roles.
-- WARNING: Do not use these policies in production. Remove or tighten before deploying.
DO $$
DECLARE tbl text;
DECLARE pol text;
BEGIN
	FOREACH tbl IN ARRAY ARRAY[
		...
		-- INSERT
		pol := 'insert_anon_' || tbl;
		IF NOT EXISTS (
			SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename=tbl AND policyname=pol
		) THEN
			EXECUTE format('CREATE POLICY %I ON public.%I FOR INSERT TO anon USING (true) WITH CHECK (true);', pol, tbl);
		END IF;
		pol := 'insert_auth_' || tbl;
		IF NOT EXISTS (
			SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename=tbl AND policyname=pol
		) THEN
			EXECUTE format('CREATE POLICY %I ON public.%I FOR INSERT TO authenticated USING (true) WITH CHECK (true);', pol, tbl);
		END IF;

		-- UPDATE
		pol := 'update_anon_' || tbl;
		IF NOT EXISTS (
			SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename=tbl AND policyname=pol
		) THEN
			EXECUTE format('CREATE POLICY %I ON public.%I FOR UPDATE TO anon USING (true) WITH CHECK (true);', pol, tbl);
		END IF;
		pol := 'update_auth_' || tbl;
		IF NOT EXISTS (
			SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename=tbl AND policyname=pol
		) THEN
			EXECUTE format('CREATE POLICY %I ON public.%I FOR UPDATE TO authenticated USING (true) WITH CHECK (true);', pol, tbl);
		END IF;

		-- DELETE
		pol := 'delete_anon_' || tbl;
		IF NOT EXISTS (
			SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename=tbl AND policyname=pol
		) THEN
			EXECUTE format('CREATE POLICY %I ON public.%I FOR DELETE TO anon USING (true);', pol, tbl);
		END IF;
		pol := 'delete_auth_' || tbl;
		IF NOT EXISTS (
			SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename=tbl AND policyname=pol
		) THEN
			EXECUTE format('CREATE POLICY %I ON public.%I FOR DELETE TO authenticated USING (true);', pol, tbl);
		END IF;
	END LOOP;
END $$;
